...
func (srv *PBServer) Start(command interface{}) (
	...
	for i := 0; i < len(srv.peers); i++ {
		go func(server int) {
			var replyArgs PrepareReply
			ok := srv.sendPrepare(server, prepArgs, &replyArgs)
			if ok {
				prepareReplyChan <- &replyArgs
			} else {
...
func (srv *PBServer) Prepare(args *PrepareArgs, reply *PrepareReply) {
	...
	if GetPrimary(srv.currentView, len(srv.peers)) != srv.me {
		if srv.currentView == args.View && srv.status == NORMAL {
			if srv.index == args.Index-1 && srv.index == args.PrimaryCommit {
				...
				recoveryReply := make(chan *RecoveryReply, len(srv.peers))
				for i := 0; i < len(srv.peers); i++ {
					go func(server int) {
						var reply RecoveryReply
						ok := srv.peers[server].Call("PBServer.Recovery", &RecoveryArgs{View: args.View, Server: srv.me}, &recoveryReply)
						if ok {
							recoveryReply <- &reply
						} else {
							recoveryReply <- nil
				...
				var successReplies []*RecoveryReply
				nReplies :=  0
				majority := len(srv.peers)/2 + 1
				for r := range recoveryReply {
					nReplies++
					if r != nil && r.Success {
						successReplies = append(successReplies, r)
					}
					if nReplies == len(srv.peers) || len(successReplies) == majority {
						srv.log = r.Entries
						srv.commitIndex = r.PrimaryCommit
						srv.status = NORMAL
					}
				}
			}
